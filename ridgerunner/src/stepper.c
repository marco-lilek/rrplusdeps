     ...
     
{
  ...
  for(gCorrectionAttempts=0;  
    ...
    //zeroForceForStaticLines(inLink, ofv_vect);
...
void zeroForceForStaticLines(plCurve *inLink, plc_vector* dVdt) {
  int cItr, vItr;
  int i = 0;
  for( cItr=0; cItr<inLink->nc; cItr++ ) {
    if (gIsLineStatic[cItr]) {
      for (vItr = 0; vItr < inLink->cp[cItr].nv; vItr++) {
        if (dVdt[i].c[0] != 0 || dVdt[i].c[1] != 0 || dVdt[i].c[2] != 0) {
          printf("zeroing %d at cItr %d vItr %d  used to be %f %f %f\n", i, cItr, vItr, dVdt[i].c[0],dVdt[i].c[1],dVdt[i].c[2]);
          //dVdt[i] = plc_build_vect(0,0,0);
        }
...

{
  ...
  zeroForceForStaticLines(inLink, dVdt);
  free(dLen);
...

{
  ...
  //strutCount = strutCount == 0 ? 0: strutCount - 1;

  int nnz = 12*strutCount + 9*minradLocs + 3*constraintCount + 3*gNumStaticPoints; // we KNOW this
  taucs_ccs_matrix *cleanA;

  printf("%d %d %d %d %d\n", 3*plc_num_verts(inLink), strutCount, minradLocs, constraintCount, gNumStaticPoints);
  cleanA = taucs_ccs_new(3*plc_num_verts(inLink),strutCount+minradLocs+constraintCount + gNumStaticPoints*3,nnz); /* (rows,columns,nnz) */
...
/*   fatalifnull_(cleanA->values.d); */
  ...
  for(sItr=strutCount+minradLocs+constraintCount+1;
      sItr<=strutCount+minradLocs+constraintCount+gNumStaticPoints*3;sItr++) {

    cleanA->colptr[sItr] = cleanA->colptr[sItr-1]+1;
  ...
  // place other stuff
  int cItr;
  int staticPointIter = 0, rowInd = 0;
  for (cItr = 0; cItr < inLink->nc; cItr++) {
    if (gIsLineStatic[cItr]) {
      int vItr = 0;
      for (vItr = 0; vItr < inLink->cp[cItr].nv; vItr++) {
        int i = 0;
        for (i = 0; i < 3; i++) {
          int colPtr = cleanA->colptr[strutCount+minradLocs+constraintCount + 3*staticPointIter + i];
          cleanA->values.d[colPtr] = -1;
          cleanA->rowind[colPtr] = rowInd;
          //printf("setting %d %d col %d to -1\n", rowInd, rowInd / 3, colPtr);
          rowInd += 1;
        }
        staticPointIter += 1;
      }
    } else {
      rowInd += inLink->cp[cItr].nv * 3;
    }
  }

/*  int colPtr = cleanA->colptr[strutCount+minradLocs];
  cleanA->values.d[colPtr + 0] =6;
  cleanA->rowind[colPtr + 0] = 4;*/
  //taucs_print_ccs_matrix(cleanA);
...

{
  ...
  if (A->n == 0 || A->n == gNumStaticPoints*3) {  /* There are no constraints yet.
    ...
    for (cItr = 0; cItr < inLink->nc; cItr++) {
      if (gIsLineStatic[cItr]) {
        int vItr = 0;
        for (vItr; vItr < inLink->cp[cItr].nv; vItr++) {
          if (dl[idx].c[0] == 0 || dl[idx].c[1] == 0 || dl[idx].c[2] == 0) {
            dl[idx] = plc_build_vect(A->n,A->n,A->n); // will get 0ed out anyways
          }
          idx++;
        }
      } else {
        idx += inLink->cp[cItr].nv;
      }
    }
...
/*      printf("%f ", minusDL[dlItr*3+0]);
      printf("%f ", minusDL[dlItr*3+1]);
      printf("%f\n", minusDL[dlItr*3+2]);*/
    }
