...
#define BYTES_PER_ENTRY 32.0
  ...
  *extptr = 0; /* This chops the extension off of basename. */
  ...
  gIsLineStatic = malloc(sizeof(bool) * link->nc);
  gNumStaticPoints = 0;
  int vItr = 0;
  for(i=0;i<link->nc;i++) {
    gIsLineStatic[i] = link->cp[i].clr[0].alpha == 0;
    gNumStaticLines += gIsLineStatic[i];
    printf("static %d\n", (int)gIsLineStatic[i]);
    int ii = 0;
    for (ii = 0; ii < link->cp[i].nv; ii++) {
      if (gIsLineStatic[i] && ii == 1) {
        printf("key point %d\n", vItr);
      }
      gNumStaticPoints += gIsLineStatic[i];
      vItr++;
    }
  }
  ...
  plCurve *tempLink = NULL;
  
  if (arg_resolution->count > 0) { 
    
    logprintf("Original curve has %d verts, %g verts/rop.\n",plc_num_verts(link), plc_num_verts(link)*octrope_thickness(link,NULL,0,gLambda)/plc_arclength(link,NULL));
  ...
  /* only print non-static lines */
/*  for (i = 0; i < link->nc; i++) {
    gIsLineStatic[i] = 0; !gIsLineStatic[i];
  }
  plCurve *cleanLink = plc_copy_some(link, gIsLineStatic);*/
  plc_write(savefile,link);
  //plc_free(cleanLink);
